...
class plugin_manager {
	...
	function plugin_loadheadfile() {
		$css=""; $js="\n";
		$query = safe_query("SELECT * FROM `".PREFIX."plugins` WHERE `activate`='1' ");
		while($res=mysqli_fetch_array($query)) {
			if(is_dir($res['path']."css/")) { $subf = "css/"; } else { $subf=""; }
				$f = array();
				$f[] = glob(preg_replace('/(\*|\?|\[)/', '[$1]', $res['path'].$subf).'*.css');
			...
			if(is_dir($res['path']."js/")) { $subf2 = "js/"; } else { $subf2=""; }
			$g = array();
			$g[] = glob(preg_replace('/(\*|\?|\[)/', '[$1]', $res['path'].$subf2).'*.js');
