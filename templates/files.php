...
#==================
        ...
        $id=mysqli_insert_id();
        $filepath = "./images/file-pics/";
        if ($screen['name'] != "") {
        move_uploaded_file($screen['tmp_name'], $filepath.$screen['name'].".tmp");
        @chmod($filepath.$screen['name'].".tmp", $new_chmod);
        $file_ext=strtolower(substr($screen['name'], strrpos($screen['name'].".tmp", ".")));
        $file=$id.$file_ext;
        rename($filepath.$screen['name'].'.tmp', $filepath.$file);
        safe_query("UPDATE ".PREFIX."files SET screen='$file' WHERE fileID='$id' ");
        }
#====================        
            }
        }
    ...
    #==============================    
        ...
        if($screen['name'] != "") {
        $file = $filepath.$screen['name'];
        if(file_exists($file)) {
            unlink($file);
        }
        
        if(move_uploaded_file($screen['tmp_name'], $file)) {
            $file=$screen['name'];
            $filesize = $screen['size'];
